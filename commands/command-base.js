    ...
    for (const alias of commands) {
      ...
      if (
        ...
        if (requiredChannel && requiredChannel !== channel.name) {
          ...
          let sameuser = [`Hmmmm... Looks like that command isn't avaible in this channel try checking`, `ERROR 404 : Command not avaible in current channel.. Try checking`]

          let same = Math.floor(Math.random() * sameuser.length);
          message.reply(sameuser [same] `<#${foundChannel.id}>.`);
        ...
        if (
          ...
          let sameuser = [`Hmm.. You gave a wrong input, I prefer you use ${prefix}${alias} ${expectedArgs}.`, `Looks like you didn't check out the help menu.. Try ${prefix}${alias} ${expectedArgs}.`, `Feeling sad you failed to use the command.. Here you go ${prefix}${alias} ${expectedArgs}, Thank me later..`]
...
//______________________________________________________________________________________________________________________________________________________________----_____________________________
/**
